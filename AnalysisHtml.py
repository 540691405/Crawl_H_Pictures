...
#传入类型url,原网址url ，
# 解析出所有单个图集名字和url,返回单个图集名字和url组成的dic
async def GetSetUrlsFromTypeUrl_async(session:aiohttp.client.ClientSession,type_url:str,source_url:str):
    #

    imgset_urls_dic = {}
    # 初始化图集信息的dic

    type_response = await session.get(type_url)
    # 进入类型网页
    type_html = await type_response.text()
    # 得到类型的网页html
...
#传入一个图集的url，解析出所有单个图片的url,返回由他们组成的list
async def GetImgUrlsFromSetUrl_async(session:aiohttp.client.ClientSession,imgset_url:str):
    #参数：已有session, 图集的url：img_set_url,图集的名称: imgset_name
    #返回 img_urls list
    img_urls=[]
    #初始化图片的url的list
    imgset_response = await session.get(imgset_url)
    # 得到一个图片集的response
    imgset_html = await imgset_response.text()
    # 得到图片集的html
    imgset_soup = BeautifulSoup(imgset_html, 'lxml')
    # 用BeautifulSoup处理图片集html
    img_tags = imgset_soup.find_all('img')
